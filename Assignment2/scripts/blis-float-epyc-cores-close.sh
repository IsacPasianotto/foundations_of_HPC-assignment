...
echo m,k,n,time,GFLOPS > ./results/blis-float-epyc-cores-close.csv
...
do
	for i in {1..15}
	do
		numactl --interleave=0-3 ./gemm_blis.x $size $size $size >> ./results/blis-float-epyc-cores-close.csv
	done
done
