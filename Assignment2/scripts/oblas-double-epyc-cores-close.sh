...
echo m,k,n,time,GFLOPS > ./results/oblas-double-epyc-cores-close.csv
...
do
	for i in {1..15}
	do
		numactl --interleave=0-3 ./gemm_oblas.x $size $size $size >> ./results/oblas-double-epyc-cores-close.csv
	done
done
