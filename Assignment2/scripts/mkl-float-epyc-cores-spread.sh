...
echo m,k,n,time,GFLOPS > ./results/mkl-float-epyc-cores-spread.csv
...
do
	for i in {1..15}
	do
		numactl --interleave=0-7 ./gemm_mkl.x $size $size $size >> ./results/mkl-float-epyc-cores-spread.csv
	done
done
