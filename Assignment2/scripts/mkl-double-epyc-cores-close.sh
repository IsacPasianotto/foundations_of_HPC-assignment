...
echo m,k,n,time,GFLOPS > ./results/mkl-double-epyc-cores-close.csv
...
do
	for i in {1..15}
	do
		numactl --interleave=0-3 ./gemm_mkl.x $size $size $size >> ./results/mkl-double-epyc-cores-close.csv
	done
done
