...
echo m,k,n,time,GFLOPS > ./results/blis-float-epyc-cores-spread.csv
...
do
	for i in {1..15}
	do
		numactl --interleave=0-7 ./gemm_blis.x $size $size $size >> ./results/blis-float-epyc-cores-spread.csv
	done
done
